 ...
 * 
 * Posibles valores:
 * 	+ $GLOBALS['results']['match'] = TRUE si encontró el usuario y la contraseña
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	if(isset($_POST['usuario']) && isset($_POST['contrasena'])){
		...
		/*
		 * Se obtiene el usuario y la contraseña por medio de las variables POST.
		 */
		...
		$queryGenerico->setSelect('id, nombre, apellidoP, apellidoM, usuario');
		$queryGenerico->setWhere('usuario = ? AND contrasena = ?');
		 ...
		 * parámetros.
		 */
		$queryGenerico->setParamsValues(array($usuario_input, $contrasena_input));
		...
		if($GLOBALS['results']){
			...
			// Se ejecuta la función de login
			iniciarSesion();

		} else {

			/*
			 * Se cambia la tabla de 'Administrador' a la del 'Vendedor'.
			 */
		...
		/*
		 * Si no se encuentran el usuario y la contraseña, se pone la llave 'match'
...
/**
 * Se extraen los datos del usuario para meterlo en la variable de la SESSION.
 * @return void  
 */
function iniciarSesion(){
	...
	if(!empty($nombre)){
		...
		$_SESSION['idAdmin'] = $GLOBALS['results'][0]['id'];
	} else {
