...
/**
 * Lista de vendedores para la tabla del Panel de Control.
 * 
 * Archivo PHP para mostrar en la tabla del Panel de Control la lista de
 * vendedores que hay en la BD. Este archivo está adaptado para el plugin de
...
for ($i=0; $i < count($resArr); $i++) { 
	...
	// Se transforman los datos obtenidos de cada columna en un div que se enviará a DataTables.
 	$sub_array[] = '<div  data-id="' . $resArr[$i]["id"] . '" data-column="vendedor">' . $resArr[$i]["nombre"] . '</div>';
 	$sub_array[] = '<div  data-id="' . $resArr[$i]["id"] . '" data-column="vendedor">' . $resArr[$i]["apellidoP"] 	  .	'</div>';
 	$sub_array[] = '<div  data-id="' . $resArr[$i]["id"] . '" data-column="vendedor">' . $resArr[$i]["usuario"] . '</div>';
